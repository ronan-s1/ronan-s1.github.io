...
function commandHandler(input : string) {
  if(input.startsWith("rm -rf") && input.trim() !== "rm -rf") {
    if (isSudo) {
      if(input === "rm -rf src" && !bareMode) {
        ...
        easterEggStyles();
        setTimeout(() => {
          writeLines(["I SAID <u>NOT</u> TO DO THAT!", "<br>"]);
        }, 200)

        setTimeout(() => {
          writeLines(["You had one job.", "<br>"]);
        }, 1200)
  ...
  switch(input) {
    ...
    case 'linkedin':
      ...
      setTimeout(() => {
        window.open('https://www.linkedin.com/in/ronan-singpurwala-b47ba2240/', '_blank');
      }, 500);
      break;
    case 'github':
      writeLines(["Redirecting to github.com...", "<br>"]);
      ...
      break;
    case 'email':
      writeLines(["Opening email...", "<br>"]);
      setTimeout(() => {
        window.open('mailto:singpurwala13@gmail.com', '_blank');
      }, 500);
      break;
    case 'rm -rf':
      if (bareMode) {
    ...
    case 'ls':
      ...
      break;
    case 'type shi':
      window.open('https://www.youtube.com/watch?v=XcK8JdVlcXY', '_blank');
      break;
    default:
      if(bareMode) {
...
function passwordHandler() {
  if (passwordCounter === 2) {
    ...
    writeLines(["<br>", "INCORRECT PASSWORD.", "PERMISSION NOT GRANTED.", "<br>", "Try looking harder, use your tools.", "<br>"])
  ...
  if (PASSWORD_INPUT.value === SUDO_PASSWORD) {
    ...
    writeLines(["<br>", "PERMISSION GRANTED.", "please do NOT <span class='command'>'rm -rf'</span> me.", "<br>"])
    revertPasswordChanges();
    isSudo = true;
    return
  } else {
